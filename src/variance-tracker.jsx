...
setError(`); setSigning(true); try { await signInWithPopup(fbAuth, googleProvider); } catch (e) { console.error(e); if (e.code === `auth/unauthorized-domain`) { setError(`이 도메인은 Firebase에 등록되어 있지 않습니다. Firebase 콘솔 → Authentication → Settings → 승인된 도메인에 현재 주소를 추가해주세요.`); } else if (e.code === `auth/popup-blocked`) { setError(`팝업이 차단되었습니다. 브라우저 설정을 확인해주세요.`); } else if (e.code === `auth/popup-closed-by-user` || e.code === `auth/cancelled-popup-request`) { setError(`);
} else {
...
const [round, setRound] = useState(`); const [subject, setSubject] = useState(`공법`); const [wrong, setWrong] = useState(`);
...
function add() {
if (!round || wrong === `) return; const w = parseInt(wrong); if (isNaN(w) || w < 0 || w > total) { alert(`틀린 개수는 0~${total} 사이여야 합니다.`); return; } const newScore = { id: uid(), date, round: parseInt(round), subject, type: `선택형`, wrong: w, total, note: note.trim() || null, }; setExamScores([...examScores, newScore]); setRound(`); setWrong(`); setNote(`);
}
function del(id) {
...
const [title, setTitle] = useState(`); const [subject, setSubject] = useState(`민사법`); const [target, setTarget] = useState(3); const [note, setNote] = useState(`);
...
setNewName(`); setShowAdd(false); } function bump(id) { setMaterials(materials.map(m => m.id === id ? { ...m, rounds: m.rounds + 1, lastDate: today } : m)); const list = materialLog[today] || []; const mat = materials.find(m => m.id === id); setMaterialLog({ ...materialLog, [today]: [...list, { id: uid(), materialId: id, name: mat?.name || `, date: today }] });
}
function dec(id) {
...
const STAR_COLOR = `#D4A437`; // 노란색(황금) — 우선순위 ★ 통일 색상
